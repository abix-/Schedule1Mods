...
| 12 (targetStation null write removed) | In test | After eMployee's STUCK-DIAG ran on save load, our SmartReset was nulling `_targetStation_k__BackingField` -- which vanilla uses for BOTH "cook here" AND "move output from here" tasks. With targetStation null, the chemist had no station to interact with and reported "nothing to do" even when output items were waiting. Removed the null write since the canonical `CanStartMix` gate now prevents new wedges from starting in the first place | Iteration 12 |
